...
pcp_factor(AST *ast) {
    ...
    if (token->sym == integernosym || token->sym == realnosym || token->sym == stringvalsym ||
...
pcp_expression(AST *ast) {
    ...
    if (pcp_accept(eqsym) || pcp_accept(neqsym) || pcp_accept(ltsym) || pcp_accept(ltesym) || pcp_accept(gtesym) ||
        pcp_accept(gtsym)) {
...
pcp_assign(AST *ast, pctoken *ltoken) {
    ...
    if (entry->type != functiontype && entry->type != integertype && entry->type != realtype &&
        entry->type != chartype && entry->type != stringtype)
