...
void pcprintsymtabnode(symtab *node, unsigned depth) {
    ...
    *c = '\0';
    ...
    while (entry) {
        switch (entry->type) {
            case integertype:
                printf("%s%s (%s @ %d // %d) : %d : %d :\n", tabs, entry->name, symtypestr[entry->type], entry->offset,
                       entry->size, entry->lineno, entry->val.ival);
                break;
            case realtype:
                printf("%s%s (%s @ %d // %d) : %d : %f\n", tabs, entry->name, symtypestr[entry->type], entry->offset,
                       entry->size, entry->lineno, entry->val.rval);
                break;
            case chartype:
                printf("%s%s (%s @ %d // %d) : %d : %c\n", tabs, entry->name, symtypestr[entry->type], entry->offset,
                       entry->size, entry->lineno, entry->val.cval);
                break;
            case stringtype:
                printf("%s%s (%s @ %d // %d) : %d : %s\n", tabs, entry->name, symtypestr[entry->type], entry->offset,
                       entry->size, entry->lineno, entry->val.str);
                break;
            default:
