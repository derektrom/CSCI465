...
*/
void pcgetnextc(char *cur, char *next, FILE *fp) {
	*cur = *next;
	*next = pcgetc(fp);
...
@param fp the FILE pointer
*/
void pcappendnext(char **b, char *cur, char *next, FILE *fp) {
...
Skips whitespace.
*/
void pcskipwhitespace(char *cur, char *next, FILE *fp) {
	int dontprint = 0;
	while (isspace(*cur)) {
...
*/
int pcgetkeyword(char *b, pcsym *sym) {
