 ...
 * @author Derek Trom
 * @author Elena Corpus
 * This is the parser program that recursively parses the scanned input file.
 */
#include "parser.h"
#include "symtab.h"
#include "ast.h"
...
@return 1 on success; 0 otherwise
*/
int pcp_expect2(pcsym sym1, pcsym sym2) {
...
@return 1 on success; 0 otherwise
*/
int sym_to_type(pcsym sym, symtype *type) {
...
int pcp_simple_expression(AST *ast) {
	AST *astsimexpr;
	AST *astaddasm;

	printf("********ENTERED simple_expression********\n");
...
int pcp_application(AST *ast, pctoken *ltoken) {
	AST *astfunccall;
...
int pcp_procedure_call(AST *ast, pctoken *ltoken) {
	AST *astproccall;
	symentry *entry = pclookupsym(ltoken->val.id);
	int params = 1;

	printf("********ENTERED procedure_call********\n");
...
int pcp_procedure_call_or_application(AST *ast, pctoken *ltoken) {
	return pcp_procedure_call(ast, ltoken) || pcp_application(ast, ltoken);
}

int pcp_assign(AST *ast, pctoken *ltoken) {
	AST *astassign;
	AST *astlval;
	symentry *entry = pclookupsym(ltoken->val.id);
...
int pcp_statement_part(AST *ast) {
	AST *aststatement;

	printf("********ENTERING statement_part********\n");
...
int pcp_formal_parameters(AST *ast) {
	...
	printf("********ENTERING formal_parameters********\n");
...
int pcp_function_declaration(AST *ast) {
	symtype type;
	AST *astfunc;
...
int pcp_procedure_declaration(AST *ast) {
	AST *astproc;

	printf("********ENTERING procedure_declaration********\n");
...
int pcp_procedure_and_function_definition_part(AST *ast) {
	printf("********ENTERING procedure_and_function_definition********\n");
...
int pcp_variable_definition(AST *ast) {
	pctokenlist tokens = {NULL, NULL};
	pctokenlist *tail = NULL;
	pctokenlist *cur;
	pctoken *val;
...
int pcp_variable_definition_part(AST *ast) {
	AST *astvar;
	printf("********ENTERING variable_definition********\n");
...
int pcp_constant_definition(AST *ast) {
	pctokenlist tokens = {NULL, NULL};
	pctokenlist *tail = NULL;
	pctokenlist *cur;
	pctoken *val;
...
int pcp_constant_definition_part(AST *ast) {
	AST *astconst;

	printf("********ENTERING constant_definition_part********\n");
